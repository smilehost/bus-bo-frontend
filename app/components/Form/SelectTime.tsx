            ...
            <div className="relative">
                <Select
                    ...
                    renderValue={(value) => {
                        ...
                        if (!value) return <span className="text-gray-400">เลือกเวลา</span>;
                        const index = parseInt(value as string, 10) - 1; // แปลงเป็น number
                        const item = timeData[index];

                        return <span className='text-[13px]'>{item.name} {item.times.join(', ')}</span>;
                    }}
                    sx={{
                    ...
                    {timeData.map((item, index) => (
                        <MenuItem value={index + 1} key={index}>
                            <div className='flex justify-between w-full'>
                                <p className='text-[13px]'>{item.name} {item.times.join(', ')}</p>
                                {open && (
