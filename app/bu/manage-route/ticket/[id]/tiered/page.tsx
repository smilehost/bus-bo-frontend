...
export default function Home() {
  ...
  return (
    <div className="p-6 overflow-x-auto">
                  ...
                  {j >= i ? (
                    <input
                      ...
                      className={`border px-1 py-0.5 w-16 text-center rounded ${
                        errorMatrix[i][j] ? 'border-red-500 bg-red-100' : ''
                      }`}
                      value={
      ...
      {resultJSON && (
        <div className="mt-6 bg-gray-100 border border-gray-300 p-4 rounded text-sm font-mono whitespace-pre-wrap">
          <h3 className="font-semibold mb-2">📦 Exported JSON:</h3>
          <pre>{resultJSON}</pre>
        </div>
      )}
    </div>
